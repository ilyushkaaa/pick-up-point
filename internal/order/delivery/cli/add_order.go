...
func (od *OrderDelivery) AddOrderDelivery(ctx context.Context, args []string) error {
	...
	if err != nil || weight <= 0 {
		return fmt.Errorf("weight must be positive float: %w", err)
	}
	price, err := strconv.ParseFloat(args[4], 64)
	if err != nil || weight <= 0 {
		return fmt.Errorf("price must be positive float: %w", err)
	}
	...
	newOrder := model.NewOrder(orderID, clientID, weight, price, expireDate)

	err = od.service.AddOrder(ctx, newOrder, packageType)
	if err != nil {
