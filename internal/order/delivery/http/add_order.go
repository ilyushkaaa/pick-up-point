...
func (d *OrderDelivery) AddOrder(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.MarshallAndWriteResponse(w, response.Result{Res: err.Error()}, http.StatusInternalServerError, d.logger)
		return
	}
	...
	response.MarshallAndWriteResponse(w, dto.NewOrderOutput(order), http.StatusOK, d.logger)
}
