...
func (d *OrderDelivery) GetUserOrders(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		response.MarshallAndWriteResponse(w, response.Error{Err: "client id was not passed"},
	...
	if err != nil {
		...
		response.MarshallAndWriteResponse(w, response.Error{Err: "client ID must be positive integer"},
			http.StatusBadRequest, d.logger)
		return
	}
	...
	if numOfLastOrders != "" {
		...
		if err != nil || numOfLastOrdersInt < 1 {
			...
			response.MarshallAndWriteResponse(w, response.Error{Err: "number of last orders must positive integer"},
				http.StatusBadRequest, d.logger)
			return
		}
	}
	...
	if ppOnly != "" {
		...
		if err != nil {
			...
			response.MarshallAndWriteResponse(w, response.Error{Err: "pp-only must be false or true"},
				http.StatusBadRequest, d.logger)
			return
		}
	}
