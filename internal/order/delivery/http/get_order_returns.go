...
func (d *OrderDelivery) GetOrderReturns(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		response.MarshallAndWriteResponse(w, response.Result{Res: "num of orders per page is not passed"},
	...
	if err != nil || ordersPerPageInt < 1 {
		...
		response.MarshallAndWriteResponse(w, response.Result{Res: "num of orders per page must be positive integer"},
			http.StatusBadRequest, d.logger)
		return
	}
	...
	if pageNum != "" {
		...
		if err != nil || pageNumInt < 1 {
			...
			response.MarshallAndWriteResponse(w, response.Result{Res: "page number must positive integer"}, http.StatusBadRequest, d.logger)
			return
		}
	}
