...
func (d *OrderDelivery) DeleteOrder(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		response.MarshallAndWriteResponse(w, response.Error{Err: "order id is not passed"},
	...
	if err != nil {
		...
		response.MarshallAndWriteResponse(w, response.Error{Err: "order ID must be positive integer"},
			http.StatusBadRequest, d.logger)
		return
	}
