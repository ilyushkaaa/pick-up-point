...
func Test_UpdatePickUpPoint(t *testing.T) {
	...
	t.Run("error validation", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"result":"Address.house_num: non zero value required;ID: non zero value required"}`, string(body))
	})
	...
	t.Run("error pick-up point with such id was not found", func(t *testing.T) {
		...
		s.mockService.EXPECT().UpdatePickUpPoint(gomock.Any(), fixtures.PickUpPoint().Valid().V()).Return(storage.ErrPickUpPointNotFound)
		respWriter := httptest.NewRecorder()
	...
	t.Run("error pick-up point with such name already exists", func(t *testing.T) {
		...
		s := setUp(t)
		request := httptest.NewRequest(http.MethodPut, "/pick-up-point", strings.NewReader(test_json.ValidPPUpdateRequest))
		s.mockService.EXPECT().UpdatePickUpPoint(gomock.Any(), fixtures.PickUpPoint().Valid().V()).Return(storage.ErrPickUpPointNameExists)
		respWriter := httptest.NewRecorder()
